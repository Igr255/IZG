...
#all necessary matrix or vector operations can be found at https://docs.blender.org/api/current/mathutils.html
    ...
    def createView():
        ...
        centerPosition = mathutils.Vector((0, 0, 0))
        #defines where is "up" in your scene, along Z axis
        upVector =  mathutils.Vector((0.0, 0.0, 0.0))      
        ...
        #vector pointing from the center to the camera will define the new Z aka depth, might be rotated like the camera
        Zrot = mathutils.Matrix.Rotation(radians(-12),4,'Z')
        Z = eyePosition @ Zrot
        Z.normalize()
    ...
    def createTransformation(object, view):
        ...
        if (transformation == view):
            Xrot = mathutils.Matrix.Rotation(radians(90),4,'X')
        else:
            Xrot = mathutils.Matrix.Rotation(radians(270),4,'X')
        
        Yrot = mathutils.Matrix.Rotation(radians(180),4,'Y')
        transformation = view @ mathutils.Matrix.Translation((3.5 - objectNumber, 0, 0))
        transformation = transformation @ Xrot
        transformation = transformation @ Yrot
    ...
    def applyTransformation(transformation, object):
        for vertex in object.data.vertices:
            ...
            #TODO: apply the transformation on vertices
            #######################
            Xrot = mathutils.Matrix.Rotation(radians(180),4,'Z')
            
            vertex.co = mathutils.Vector(vertex.co)
            vertex.co = transformation @ vertex.co
            vertex.co = vertex.co @ Xrot
